...
type Chunk = { id: string; doc: string; source: string; text: string; embedding: number[] };
...
export function loadEmbeddingIndex(): Chunk[] {
  ...
  if (!fs.existsSync(p)) throw new Error("No embeddings.json. Run buildEmbeddingIndex() first.");
